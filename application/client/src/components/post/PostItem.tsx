...
import { formatDateJaLong, toISOString } from "@web-speed-hackathon-2026/client/src/utils/datetime";
    ...
    <article className="px-1 sm:px-4">
      <div className="border-cax-border border-b px-4 pt-4 pb-4">
        ...
        <div className="pt-2 sm:pt-4">
          ...
          {post.movie ? (
            <div className="relative mt-2 w-full">
              <MovieArea movie={post.movie} priority={true} />
            </div>
          ) : null}
          {post.sound ? (
            <div className="relative mt-2 w-full">
          ...
          <p className="mt-2 text-sm sm:mt-4">
            <Link className="text-cax-text-muted hover:underline" to={`/posts/${post.id}`}>
              <time dateTime={toISOString(post.createdAt)}>
                {formatDateJaLong(post.createdAt)}
              </time>
            </Link>
          </p>
        </div>
